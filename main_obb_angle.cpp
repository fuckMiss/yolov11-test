...
int main(int argc, char** argv)
{
    ...
    if (isVideo) {
        ...
        while (1) {
            ...
            DrawOBBAngleOverlay(image, objects, config.class_names);
        ...
        for (const auto& imagePath : imagePathList) {
            ...
            string output_path = "obb_angle_result_" + imagePath.substr(imagePath.find_last_of("/\\") + 1);
            imwrite(output_path, image);
            printf("Saved angle result to: %s\n", output_path.c_str());
